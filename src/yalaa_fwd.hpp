 ...
 * \c YalAA is built with CMake (>= 3.16) and works on any platform with a C++11 compiler,
 * such as
 * \li GNU GCC
 * \li CLANG++
 ...
 * \li Configure the build, pointing CMake at your interval backend's install prefix <br>
 * (Example: <tt>cmake -S . -B build -DYALAA_IV_BACKEND=cxsc -DCXSC_ROOT=<CXSC-PATH></tt>)
 * \li Build the library (<tt>cmake --build build</tt>)
 * \li Install the library (<tt>cmake --install build</tt>) <br>
 * <i>If desired, pass \c -DCMAKE_INSTALL_PREFIX=<path> at configure time to install \c YalAA
 * in a custom location, e.g. your \c $HOME directory</i>
 * 
 * \subsection Documentation
 * For creating the documentation you need <a href="http://www.stack.nl/~dimitri/doxygen/">Doxygen</a>.
 * \li Adjust the output format in the \c Doxyfile if desired
 * \li Start the creation process (<tt>doxygen Doxyfile</tt>)
