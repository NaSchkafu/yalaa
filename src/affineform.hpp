...
#define YALAA_FRIEND_DECL   template<typename AF>			\
  ...
  namespace details
  {
    YALAA_DEFINE_HAS_TRAIT(trait_is_aff_t)
  }
  ...
  public:
    ...
    // (previously generated by boost::arithmetic/subtractable2_left/
    //  dividable2_left; implemented here as hidden friends in terms of
    //  the compound-assignment operators above)
    // ****************************************************************
    friend self_t operator+(self_t lhs, const self_t &rhs) { lhs += rhs; return lhs; }
    friend self_t operator-(self_t lhs, const self_t &rhs) { lhs -= rhs; return lhs; }
    friend self_t operator*(self_t lhs, const self_t &rhs) { lhs *= rhs; return lhs; }
    friend self_t operator/(self_t lhs, const self_t &rhs) { lhs /= rhs; return lhs; }

    friend self_t operator+(self_t lhs, const iv_t &rhs) { lhs += rhs; return lhs; }
    friend self_t operator+(const iv_t &lhs, self_t rhs) { rhs += lhs; return rhs; }
    friend self_t operator-(self_t lhs, const iv_t &rhs) { lhs -= rhs; return lhs; }
    friend self_t operator-(const iv_t &lhs, const self_t &rhs) { self_t tmp(lhs); tmp -= rhs; return tmp; }
    friend self_t operator*(self_t lhs, const iv_t &rhs) { lhs *= rhs; return lhs; }
    friend self_t operator*(const iv_t &lhs, self_t rhs) { rhs *= lhs; return rhs; }
    friend self_t operator/(self_t lhs, const iv_t &rhs) { lhs /= rhs; return lhs; }
    friend self_t operator/(const iv_t &lhs, const self_t &rhs) { self_t tmp(lhs); tmp /= rhs; return tmp; }

    friend self_t operator+(self_t lhs, base_ref_t rhs) { lhs += rhs; return lhs; }
    friend self_t operator+(base_ref_t lhs, self_t rhs) { rhs += lhs; return rhs; }
    friend self_t operator-(self_t lhs, base_ref_t rhs) { lhs -= rhs; return lhs; }
    friend self_t operator-(base_ref_t lhs, const self_t &rhs) { self_t tmp(lhs); tmp -= rhs; return tmp; }
    friend self_t operator*(self_t lhs, base_ref_t rhs) { lhs *= rhs; return lhs; }
    friend self_t operator*(base_ref_t lhs, self_t rhs) { rhs *= lhs; return rhs; }
    friend self_t operator/(self_t lhs, base_ref_t rhs) { lhs /= rhs; return lhs; }
    friend self_t operator/(base_ref_t lhs, const self_t &rhs) { self_t tmp(lhs); tmp /= rhs; return tmp; }
